...
#else
#define XBOX_CODELINE_ONLY() \
        Error_Compiling_Code_Only_Valid_in_Xbox_Codeline
#endif
...
// This is the preferred Min operator. Using the MIN macro can lead to
// unexpected side-effects or more expensive code.
template <class T>
...
// This is the preferred Max operator. Using the MAX macro can lead to
// unexpected side-effects or more expensive code.
template <class T>
...
#define DEFINE_ENUM_BITWISE_OPERATORS(Type)  \
    ...
    inline Type operator<<(Type a, int b)    \
    {                                        \
        return Type(int(a) << b);            \
    }                                        \
    inline Type operator>>(Type a, int b)    \
